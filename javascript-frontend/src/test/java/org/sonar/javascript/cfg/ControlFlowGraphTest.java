...
public class ControlFlowGraphTest {
  ...
  public void and_condition() throws Exception {
    ControlFlowGraph g = build("if (b0 && b1) { b2; } b3;", 5);
    assertBlock(g, 0).hasElements("b0").hasSuccessors(2, 1).hasBranchingTree(Kind.CONDITIONAL_AND);
    assertBlock(g, 1).hasElements("b0 && b1").hasSuccessors(3, 4).hasBranchingTree(Kind.IF_STATEMENT);
    assertBlock(g, 2).hasElements("b1").hasSuccessors(1);
    assertBlock(g, 3).hasElements("b2").hasSuccessors(4);
    assertBlock(g, 4).hasElements("b3").hasSuccessors(END);
  }

  @Test
  public void or_condition() throws Exception {
    ControlFlowGraph g = build("if (b0 || b1) { b2; } b3;", 5);
    assertBlock(g, 0).hasElements("b0").hasSuccessors(1, 2).hasBranchingTree(Kind.CONDITIONAL_OR);
    assertBlock(g, 1).hasElements("b0 || b1").hasSuccessors(3, 4).hasBranchingTree(Kind.IF_STATEMENT);
    assertBlock(g, 2).hasElements("b1").hasSuccessors(1);
    assertBlock(g, 3).hasElements("b2").hasSuccessors(4);
    assertBlock(g, 4).hasElements("b3").hasSuccessors(END);
  ...
  public void assignment_with_and() throws Exception {
    ControlFlowGraph g = build("x = b0 && b1 && b2;", 5, 0);
    assertBlock(g, 0).hasElements("x", "b0").hasSuccessors(3, 1);
    assertBlock(g, 2).hasElements("b0 && b1 && b2", "x = b0 && b1 && b2").hasSuccessors(END);
    assertBlock(g, 3).hasElements("b1").hasSuccessors(1);
    assertBlock(g, 1).hasElements("b0 && b1").hasSuccessors(4, 2);
  }

  @Test
  public void assignment_with_or() throws Exception {
    ControlFlowGraph g = build("x = b0 || b1 || b2;", 5, 0);
    assertBlock(g, 0).hasElements("x", "b0").hasSuccessors(1, 3);
    assertBlock(g, 2).hasElements("b0 || b1 || b2", "x = b0 || b1 || b2").hasSuccessors(END);
    assertBlock(g, 3).hasElements("b1").hasSuccessors(1);
    assertBlock(g, 1).hasElements("b0 || b1").hasSuccessors(2, 4);
  }
